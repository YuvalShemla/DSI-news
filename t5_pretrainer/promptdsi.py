...
from t5_pretrainer.eprompt import EPromptWithTopicModelling
...
from t5_pretrainer.utils.utils import print_params_info
...
class PromptDSIAttention(nn.Module):
    ...
    def forward(
        ...
        if prompt is not None:
            ...
            assert (
                ...
                key_prefix.shape[1],
                key_prefix.shape[3],
            ) == expected_shape, f"key_prefix.shape: {key_prefix.shape} not match k.shape: {key_states.shape}"
            assert (
                value_prefix.shape[0],
                value_prefix.shape[1],
                value_prefix.shape[3],
            ) == expected_shape, f"value_prefix.shape: {value_prefix.shape} not match v.shape: {value_states.shape}"
            key_states = torch.cat([key_prefix, key_states], dim=2)
            value_states = torch.cat([value_prefix, value_states], dim=2)
...
class PromptDSILayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = PromptDSIAttention(
...
class MixloraT5Block(nn.Module):
    def __init__(self, config, mixlora_config, has_relative_attention_bias=False, name=None, num_experts=8):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.layer = nn.ModuleList()
        self.layer.append(
            PromptDSILayerSelfAttention(
                config, has_relative_attention_bias=has_relative_attention_bias
            )
        )
        if self.is_decoder:
...
class MixloraT5Stack(T5PreTrainedModel):
    def __init__(self, config, mixlora_config, embed_tokens=None):
        ...
        self.is_decoder = config.is_decoder
        self.e_prompt = EPromptWithTopicModelling()
        ...
        for i, (layer_module, past_key_value) in enumerate(
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if i == 0:
                    ...
                    res = self.e_prompt(cls_features)
                    prompts = res["batched_prompt"][0]

                if i == 1:
                    prompt_length = prompts.shape[2]
                    _extended_attention_mask = torch.cat(
                        [
                            torch.zeros(
                    ...
                    _extended_attention_mask = torch.cat(
                        [
                            torch.zeros(
                                extended_attention_mask.shape[0],
                                1,
                                _extended_attention_mask.shape[-2],
                                prompt_length,
                                device=extended_attention_mask.device,
                            ),
                            _extended_attention_mask,
                    ],
                        dim=-1,
                    )
                    _position_bias = self.block[0].layer[0].SelfAttention.compute_bias(query_length=hidden_states.shape[1],key_length=hidden_states.shape[1] + prompt_length,)
...
class PromptDSI(Module): # Now implemented a masked head    
    ...
    def __init__(self, model_name_or_path, mixlora_config, base_model_cls=MixloraT5ForSemanticGeneration):
        ...
        # Losses
        self.rank_loss_fn = torch.nn.CrossEntropyLoss() #label_smoothing=0.1
        # self.rank_loss_fn = nn.NLLLoss()
    ...
    def _freeze_base_model(self, freeze_vocab=True):
        mixlora_components = ["prompt.prompt", "to_gates", "gate_"]
        if not freeze_vocab:
            mixlora_components.append("shared") # Adding shared to trainable params
        for name, param in self.base_model.named_parameters():
            if "shared" in name or ("prompt" in name and not "key" in name):
                param.requires_grad = True
            else:
                param.requires_grad = False
        print("##### Params info after freezing base model #####")
        get_params_info(self.base_model)
        print("##### Params info after freezing base model #####")

    def _before_task(self):
        self._freeze_base_model(freeze_vocab=False)
