...
def load_and_evaluate(qrel_file_path, run_file_path, metric):
    ...
    if "TREC" in qrel_file_path:
        assert ("binary" not in qrel_file_path) == (
            metric == "ndcg" or metric == "ndcg_cut"
        )
    if metric == "mrr_10":
...
def load_and_evaluate_for_qid_smtid(
    qrel_file_path, run_file_path, metric, docid_to_smtid_path, truncate_smtid=False
):
    qrel = from_qrel_to_qsmtid_rel(
        docid_to_smtid_path, qrel_file_path, truncate_smtid=truncate_smtid
    )
    with open(run_file_path) as reader:
        run = json.load(reader)
...
def init_eval(metric):
    ...
    if metric == "MRR@10":
        ...
        return lambda x, y: evaluate(
            x, y, metric="recall", agg=True, select=metric.split("@")[1]
        )
...
def get_fsmtid_hit_rate(batch_qids, prefx_smtids, qid_to_reldata, docid_to_smtids):
    ...
    for qid, psmtid in zip(batch_qids, prefx_smtids):
        ...
        if fsmtid in rel_fsmtids:
            qid_to_hitrate[qid] = 1.0
        else:
            qid_to_hitrate[qid] = 0.0
