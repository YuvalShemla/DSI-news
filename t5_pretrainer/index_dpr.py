...
import ujson
from t5_pretrainer.dataset import CollectionDatasetPreLoad, DataLoaderWrapper
from t5_pretrainer.losses.regulariaztion import L0, L1
from t5_pretrainer.ripor import BertDenseEncoder
...
def dense_indexing_dpr(args):
    ddp_setup()

    # # parallel initialiaztion
    assert local_rank != -1
    model = BertDenseEncoder.from_pretrained(args.pretrained_path)
    missing, _ = model.base_model.load_state_dict(torch.load("./mixlora_dsi/msmarco/dpr/output/ctx_model.bin"))
...
def dense_retrieve_dpr(args):
    model = BertDenseEncoder.from_pretrained(args.pretrained_path)
    missing, _ = model.base_model.load_state_dict(torch.load("./mixlora_dsi/msmarco/dpr/output/question_model.bin"))
    print("Missing", missing)
    model.cuda()
